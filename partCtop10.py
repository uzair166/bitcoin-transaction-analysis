...
#joined_data[1][0][1] = voutwiki.n (id for output within this transaction)
#joined_data[1][0][2] = voutwiki.publicKey (id of wallet where coins are being sent)
joined_data1 = joined_data.map(lambda f: ((f[1][1][0]),(f[0],f[1][0][0],f[1][0][1],f[1][0][2],f[1][1][1])))
...
vout_join = vout_f.map(lambda fields: ((fields[0]),(fields[1],fields[3],fields[2])))
...
newer_joined_data = new_joined_data.map(lambda f: ((f[1][0][0]),   (f[1][1][1],f[1][0][1],f[1][1][0])))

groupeddata = newer_joined_data.groupByKey()

addresses = groupeddata.map(finalfilter)
sorted = addresses.sortBy(lambda a: -a[1])
sorted.saveAsTextFile("sorted")
